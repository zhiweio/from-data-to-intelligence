...
!!! abstract "项目第 4 年 · Data+AI 转型期——POC 架构总览"
...
[Ch 38](./38-时代命题-AI-Ready数据供应.md) 把"为什么要做 Agentic BI"讲清楚了，这一章展开讲 **POC 里架构怎么设计**。

先把 Part VII 的坐标钉死：NewtonData 是第 4 年的 **方向探索 + POC 验证**——我们用一个较小业务域的数据把链路跑通，上线给内部用户体验，不是拿全书当教学作业，也不是声称已经覆盖 CDP 全部业务域。后面画的双平面、五层模型、9 步查询流，都是这次 POC 里立住的骨架；推到单域上千张表、全平台生产，还要过评估与演进（[Ch 49](./49-评估-可观测与持续演进.md)）。
...
!!! warning "Trade-off"
    9 步比 4 步慢了不少——端到端延迟从 2 秒涨到 3-8 秒。但换来的是：错误率从 PoC 的 ~30% 降到生产的 <5%，危险 SQL 拦截率 >99%（语法层 + 黑名单 + AST 白名单 + 成本护栏四层防御），但提示注入等新型攻击向量仍是开放问题，无法保证绝对归零，审计可追踪。在医药行业，"慢一点但对"远比"快但可能错"重要。这也是为什么我们宁可牺牲延迟，也要把检索、规划、护栏三道工序加进去。
