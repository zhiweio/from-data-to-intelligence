# Ch 52 排障与可观测性实战
!!! info "面包屑"
    [本书主页](./index.md) › [Part VIII 治理与复盘](./51-日志-监控-审计与告警.md) › Ch 52
...
```
<p class="caption" markdown="span">**图 52-1** 排障决策树</p>
...
<p class="caption" markdown="span">**表 52-1** 高频问题速查表</p>
...
flowchart LR
 ...
 Q1 -->|Redshift 没有|ELT[查 Raw→Redshift SQL ELT<br/>质检是否阻断/SQL 是否失败]
...
classDef bpInfo fill:#f6f2ff,stroke:#8a3ffc,stroke-width:2px,color:#161616
classDef bpGroup fill:#ffffff,stroke:#0f62fe,stroke-width:2px,color:#161616
class MISS,SRC,LANDING,ELT,RLS bpProcess
class Q1 bpDecision
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 52-2** 数据缺失的排查流程</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 52-3** 跨账号同步的已知边界排障</p>
...
<p class="caption" markdown="span">**表 52-2** 跨账号同步的已知边界排障</p>
...
!!! tip "引申：双联章说明"
    本章与 [Ch 51 日志、监控、审计与告警](./51-日志-监控-审计与告警.md) 合构"监控排障双联章"——Ch 51 讲"怎么看见"，本章讲"怎么排查"。建议两章连读。

---

## 52.4 排障演练：fact_prescription 数据突然为空
...
```
<p class="caption" markdown="span">**图 52-4** 排障演练：fact_prescription 数据突然为空</p>
...
**第二步：发现矛盾**（08:10）。任务 status=success，但 row_count=0——这就是 [Ch 51](./51-日志-监控-审计与告警.md) 提到的"ETL 成功但数据为空"。任务没挂，是"跑了但没拉到数据"。
...
**第六步：复盘**（08:50）。行数对账通过，事件关闭。改进措施：水位更新环节加了"不允许写未来时间"的校验。这也是 [Ch 54](./54-架构师的复盘-取舍遗憾与主流对比.md) 里"工程诚实"的具体落脚：把排障中吃到的教训固化为防护栏杆，下次同样操作会被拦住。
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 52-5** 性能退化排障：查询延迟从 3 秒涨到 30 秒</p>
...
<p class="caption" markdown="span">**表 52-3** 性能退化排障：查询延迟从 3 秒涨到 30 秒</p>
...
<p class="caption" markdown="span">**表 52-4** 预防性排障：在事故前发现问题</p>
...
!!! tip "引申"
    预防性排障的哲学是"把排障从被动变为主动"——与其等业务方反馈"数据不对/查询慢"，不如让平台自己先发现。这需要投入建设监控基线（7 天/30 天基线、SLA 定义、容量阈值），初期看似"多做事"，但每预防一次事故省下的排障时间远超建设成本。这是 [Ch 54](./54-架构师的复盘-取舍遗憾与主流对比.md) "可观测优先"原则的落地。
...
- 数据缺失按链路逐层定位：Landing→Raw→Redshift，用批次标识精确定位丢失环节
...
!!! quote "下一章"
    [Ch 53 价值度量与案例复盘](./53-价值度量与案例复盘.md) —— 排障讲完了，接下来看平台创造了什么价值——量化度量与案例复盘。
