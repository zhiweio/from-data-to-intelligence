...
跨账号同步（[Ch 32](./32-跨账号批量同步-双桶桥接架构.md)）涉及复杂的任务依赖：DDL 克隆→UNLOAD→:simple-rclone: rclone 复制→COPY→校验，多张表之间还有外键依赖。Step Functions 虽然能编排，但对于"大量同类任务 + 复杂依赖图"的场景不够灵活。平台自研了一个轻量 DAG 调度器。
...
!!! warning "Trade-off"
    自研 DAG 是"轻量"的——单线程调度、轮询模型，没有 Airflow 的分布式调度和丰富插件生态。但对于"跨账号同步"这种"数十张表 × 五步流程"的批量任务，轻量 DAG 足够用且更可控。引入 Airflow 的运维成本（维护 Airflow 实例）对于这个场景过重。
...
!!! tip "引申"
    错误传播语义是 DAG 调度器的核心设计点。现实中的任务图很少是"纯串行"或"纯并行"——通常是"部分串行 + 部分并行 + 汇聚点"的混合。failOnError 策略让调度器能灵活处理"关键路径失败中止"和"非关键路径失败继续"两种场景。
...
!!! warning "Trade-off"
    自研 DAG 的核心价值是"零运维 + 嵌入式"——它作为跨账号同步工具的一部分运行，不需要单独维护一个 Airflow 实例。但如果平台有大量不同场景需要编排（ETL + 同步 + 导出 + 告警），引入 Airflow/Dagster 统一管理更合理。本书平台的主编排用 Step Functions，自研 DAG 仅用于"跨账号同步"这个特定场景的嵌入式编排——两者各司其职。
...
!!! tip "引申"
    调度器可观测性常被忽略——"调度器自己跑得好好的，为什么要监控它？"但当源库不可达时，所有任务会堆积在 ready 队列里"假运行"，如果没有 `ready_queue_depth` 监控，这个问题要到"数据没出来"才被业务感知。调度器是数据平台的"心脏"，心脏的健康必须被监控——这是 [Ch 49](./49-日志-监控-审计与告警.md) 可观测体系在编排层的延伸。
