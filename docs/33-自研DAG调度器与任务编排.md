...
!!! warning "Trade-off"
    ...
    我得先澄清：下面三点不是"Airflow 做不到"，而是"在跨账号同步工具这个特定场景下，Airflow 的解法偏重"。一是**嵌入式运行**——同步工具是独立脚本，需在 Glue Job 里直接调用；而 Airflow 是编排器，典型用法是从外部触发 Glue Job（GlueJobOperator），不在 Glue 内反向调用 Airflow。本书"嵌入式"需求是同步工具场景的特殊约束，并非 Airflow 缺陷。二是**大量同类任务**——100 张表 × 5 步看似 500 个节点，但 Airflow 2.3+ 支持 Dynamic Task Mapping（task.expand），可用 5 个 task group + 1 个 expand 表达，无需手写 500 个任务。三是**跨账号凭证管理**——Airflow Connection 的 extra 字段支持任意 JSON，可存双账号凭证；或用 AWS Connection + AssumeRole，并非不支持双账号。

    那为什么还自研？真实价值不在"Airflow 不行"，而在**零外部依赖、与现有 Lambda/Glue 同进程、200 行 Python 解决同步工具场景的轻量编排需求**——同步工具本身是独立交付物，引入 Airflow 意味着额外部署一套调度服务（哪怕托管 MWAA 也要付 worker 实例费），对这个嵌入式轻量场景是过度工程。但对于跨业务域、多团队、需复杂数据依赖的大型编排场景，Airflow / Dagster 仍是更成熟的选择。本书平台的主编排用 Step Functions，自研 DAG 仅用于跨账号同步——两者各司其职。如果未来平台要统一编排 ETL + 同步 + 导出 + 告警等多种场景，我会考虑引入 Dagster——它的"资产驱动"理念（见 [Ch 10 引申](./10-编排与调度设计-StepFunctions与EventBridge.md)）和数据平台的可观测性诉求很契合。自研是当前约束下的选择，不代表永远不要成熟框架——架构是演进的。
...
!!! tip "引申"
    调度器可观测性常被忽略——"调度器自己跑得好好的，为什么要监控？"但当源库不可达时，所有任务会堆积在 ready 队列里假运行，如果没有 `ready_queue_depth` 监控，这个问题要到数据没出来才被业务感知。调度器是数据平台的心脏，心脏健康必须被监控——这也是 [Ch 51](./51-日志-监控-审计与告警.md) 可观测体系在编排层的延伸。
