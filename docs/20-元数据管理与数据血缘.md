...
上面的 ER 图定义了元数据的结构，落到 Redshift 就是一组建表 DDL——`batch_id` 贯穿四张表，是血缘拼接的纽带：
...
!!! tip "引申"
    元数据是数据平台的"神经系统"——没有它，平台就是黑箱，出了问题只能靠"猜"。好的元数据体系让平台"可观测、可追溯、可审计"，这是企业级平台与"玩具项目"的本质区别。
...
!!! tip "引申"
    主动血缘是数据平台的"下一代能力"。本书平台的审计日志是"被动血缘"——能追溯"发生了什么"，但不能自动回答"如果我改了 dim_hospital，哪些下游表会受影响？"。如果今天重建，建议从第一天就集成 OpenLineage——事后补血缘比一开始就收集难十倍。这也是我们在 [Ch 52](./52-架构师的复盘-取舍遗憾与主流对比.md) 列为"遗憾"之一的原因。

### 从被动血缘到主动血缘：演进路线图

被动血缘不是"错"，而是"阶段性够用"——项目初期审计日志能追溯排障，性价比最高。但随着平台规模增长（20000+ 张表），被动血缘的"手动拼接"成本线性上升，主动血缘的价值开始超过集成成本。这是一条三阶段演进路线：
...
!!! tip "引申"
    这条路线图最大的教训是**阶段一就该为阶段三埋种子**——哪怕初期不上 OpenLineage，也应在审计日志里显式记录"本任务读了哪些表、写了哪些表"（即 INPUT/OUTPUT 字段），而非只记"批次开始/结束"。这样到阶段三接入 OpenLineage 时，历史数据可回填，而非从零开始。本书平台恰恰在阶段一漏了这步，导致 [Ch 52](./52-架构师的复盘-取舍遗憾与主流对比.md) 把"无主动血缘"列为遗憾——事后补血缘难十倍，根因就在这里。
