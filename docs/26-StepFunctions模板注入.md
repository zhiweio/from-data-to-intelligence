...
!!! tip "引申"
    模板化是"DRY 原则"（Don't Repeat Yourself）在 IaC 中的体现。硬编码看似简单，但随着状态机数量增长，三份 JSON 的同步成本指数级上升。模板化的前期投入会在规模扩大后回报你。

    我从硬编码改到模板化，是被"三份 JSON 不一致"的事故逼的。最初三个环境各有一份独立的状态机 JSON——dev/qa/prod 手动维护。有次我改了 dev 的状态机（加了一个错误处理步骤），忘了同步到 qa 和 prod——结果 qa 和 prod 跑的是旧逻辑（无错误处理），一个本该重试的故障直接失败了。排查时发现三份 JSON 的"步骤数量"都不一样——dev 有 8 步，qa 有 7 步，prod 有 7 步。从那以后我改用模板化——一份模板，三个环境只是参数不同，逻辑必然一致。**硬编码的"直观"是假象——三份手动同步的 JSON 注定会不一致，只是时间问题**。模板化的前期学习成本（理解占位符和渲染）是一次性的，而硬编码的同步成本是持续的、指数增长的。
