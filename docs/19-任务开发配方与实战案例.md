...
下面是一个新 JDBC 源表全量入湖的端到端配置走读——它把 [Ch 12](./12-配置驱动的任务模型.md)（配置模型）、[Ch 14](./14-数据库与JDBC连接器.md)（JDBC 分区/水位）、[Ch 17](./17-Landing到Raw到Enriched开发实战.md)（三层加工/质检/脱敏）全串成一份任务声明：
...
这份配置发布到 config-store 后，调度器到点就会触发完整链路——连接器读取 → JDBC 分区取数 → 三层加工 → 质检门禁 → 脱敏 → 入仓——**零代码**，配置驱动。前面 8 章的设计思想，到这里汇聚成一个 Recipe。
