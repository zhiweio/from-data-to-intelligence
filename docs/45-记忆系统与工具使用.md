...
!!! tip "引申：缓存策略对比"
    ...
    NewtonData 选 SQL-Only 是因为医药数据敏感——宁可命中缓存仍执行 SQL，也不要返回脏数据。对历史报表等低新鲜度场景，可演进为分级缓存（[Ch 47](./47-评估-可观测与持续演进.md) Roadmap）。
...
Analytical Agent 根据查询结果的数据形状分流——这是"用对工具"的关键设计：
...
NewtonData 当前用 L1（Python exec + 白名单），这是最弱的隔离——白名单绕过（如通过 `__import__`、`__builtins__`、`__subclasses__`）仍可能逃逸。生产级沙箱至少应到 L2（进程隔离）或 L3（容器隔离），或用托管方案如 [E2B](https://e2b.dev/)。

---

## 45.4 可视化推荐引擎

Agent 执行 SQL 返回数据后，还要推荐合适的图表。NewtonData 用"规则优先、LLM 兜底"的混合设计——95%+ 的场景无需 LLM，规则匹配即可：
...
产出 `chart_type + echarts_option + confidence + checkpoint_status`（auto_pass/needs_review/blocked）。低置信度触发前端 HITL 候选确认——这是 human-in-the-loop 在可视化的体现。
