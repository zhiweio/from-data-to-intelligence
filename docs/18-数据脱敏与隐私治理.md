...
这张表在 [Ch 1](./01-数字化转型下的医药数据困局.md) 出现过"法规要求"视角，这里补"工程实现"视角——每项原则在本章跟前序章节怎么落地。"可归属 Attributable"靠审计日志（§18.2.3 DSR 审计报告 + [Ch 11](./11-配置与状态管理.md) 审计日志体系）；"原始 Original"靠 Landing 层不可变副本（[Ch 7](./07-数据湖分层设计.md) 分层设计）；"准确 Accurate"靠 PyDeequ 质量校验（[Ch 17](./17-Landing到Raw到Enriched开发实战.md) 质量框架）。这些不是事后对照，是我设计每一层时就按 ALCOA+ 反向推导——"这个设计满足哪几条 ALCOA？"。第四年 GxP 审计，审计员看到这张"原则→实现"映射表，说"这是我见过最系统的 ALCOA 落地"——每条都能追溯到具体的工程机制，不是空喊"我们重视数据完整性"。**合规不是 PPT 里的口号，是嵌在每一层架构里的工程机制**（M10 合规从第一天嵌入）。
