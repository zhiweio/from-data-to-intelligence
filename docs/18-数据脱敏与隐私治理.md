...
!!! info "面包屑"
    [本书主页](./index.md) › [Part III 数据工程实践](./17-Landing到Raw到Redshift开发实战.md) › Ch 18
...
所以在 Aurora 平台设计之初，我就在 [Ch 2](./02-从需求到蓝图：一个数据平台的诞生.md) 的范围边界里把"治理合规"列为平台五点核心诉求之一。这一章就是把"治理合规"从口号变成工程实现——匿名化策略矩阵、配置驱动脱敏、RLS/CLS/脱敏三层协同防护。
...
flowchart LR
    ...
    subgraph 配置驱动脱敏["配置驱动的字段级脱敏"]
        ...
        ENGINE[SQL ELT / UDF<br/>写入 enriched_* 时执行]
        UDF[脱敏 UDF<br/>mask/partial/encrypt/hash]
    end

    CONFIG --> ENGINE
    ENGINE -->|按规则调用| UDF
    UDF -->|输出脱敏后的值| ENRICHED[Redshift enriched_*<br/>已脱敏数据]
...
但"哪个字段用哪种策略"不能拍脑袋——得看字段的敏感级别、分析用途、要不要还原、要不要跨源关联。下面这张**医药行业字段脱敏决策表**把 18.1 引申框里的三个问题（要分析吗？要还原吗？要跨源关联吗？）落到具体字段，作为脱敏规则配置的依据：
...
上面的 `aes_encrypt` UDF 用到了**信封加密（Envelope Encryption）**：一个客户主密钥（CMK，永不出 KMS）加密数据密钥（Data Key），数据密钥在内存里加密数据，用完即焚。密钥从不随数据落盘，也从不硬编码在 ETL 代码里：
...
flowchart LR
    ...
    DK -->|加密| DATA[敏感字段密文<br/>落 enriched_* / 可选落盘]
    DK -.->|用完即焚| X[不落盘]
...
| **Data Key（数据密钥）** | 加密字段值 | 仅内存，用完即焚 | UDF / SQL ELT 进程内 |
| **密文字段** | 落 `enriched_*` | Redshift（+ 可选 S3 备份） | 分析师（密文不可读） |
...
PIPL 赋予患者"删除权"等数据主体权利（DSR, Data Subject Request）。当患者行使删除权时，平台不能只删一张表——患者 ID 散落在 Landing/Raw/Redshift 多层多表。DSR 响应需要一套**基于血缘的分层删除流程**：
...
| **② 分层删除** | Redshift 软删除 → Raw 分区处理 → Landing 保留但标记 | 从消费端到源头逐层清理；Landing 保留以满足 GxP 审计；已无 S3 Enriched 步 |
| **③ 审计报告** | 生成含时间戳、操作者、影响行数的 DSR 报告 | 供合规审查，证明删除已执行 |
<p class="caption" markdown="span">**表 18-4** DSR 数据主体权利响应</p>


!!! tip "引申"
    DSR 响应是"被动血缘"价值最直接的体现——如果没有血缘，DSR 只能靠人工猜"患者数据可能在哪几张表"，漏删风险极高。这也是为什么 [Ch 20](./20-元数据管理与数据血缘.md) 和 [Ch 54](./54-架构师的复盘-取舍遗憾与主流对比.md) 都把"主动血缘"列为遗憾——血缘不只是排障工具，更是合规基础设施。
...
| **不可绕过** | SQL ELT 写入 `enriched_*` 时强制执行，确保下游拿到的是脱敏后数据 |
<p class="caption" markdown="span">**表 18-6** 示意：脱敏验证——重识别攻击测试</p>


!!! warning "Trade-off"
    脱敏在入仓 SQL ELT 执行意味着 Raw 层仍保留原始敏感数据。这是必要的：如果脱敏后发现策略错了（比如该哈希的用了 mask），需要从 Raw 经 Spectrum 重跑 ELT。Raw 层的敏感数据通过 IAM 权限严格控制（只有 ETL 引擎能访问），不暴露给分析师；BI 只授权 `enriched_*`。

---

## 18.3 Redshift RLS/CLS 与脱敏 UDF 的协同防护体系

在讲三层纵深防御之前，得先说清 **PII 发现**。脱敏的前提是"知道哪些字段是 PII"。结构化字段（患者姓名、身份证、电话）好办，决策表（表 18-2）能覆盖；但医药平台还有大量 PII 散落在 S3 的自由文本里：患者反馈附件、邮件正文、客服记录，这些没法靠人工盘点。我们在第一年引入了 **AWS Macie**（原生 NER + PII 发现服务，cn-north-1 可用），让它自动扫描 S3 桶，识别患者姓名、身份证号、电话、病历号等 PII 实体，输出分类分级台账。已知结构化字段走 §18.2 的 UDF 脱敏；自由文本字段靠 Macie 持续监控，一旦某份附件里新出现了 PII，就告警并触发人工处置。没有这层扫描，自由文本里的 PII 基本是黑盒。

结合 [Ch 8](./08-数据仓库设计-Redshift.md) 介绍的 RLS/CLS，平台构建了三层纵深防御：
...
这张表在 [Ch 1](./01-数字化转型下的医药数据困局.md) 出现过"法规要求"视角，这里补"工程实现"视角——每项原则在本章跟前序章节怎么落地。"可归属 Attributable"靠审计日志（§18.2.3 DSR 审计报告 + [Ch 11](./11-配置与状态管理.md) 审计日志体系）；"原始 Original"靠 Landing 层不可变副本（[Ch 7](./07-数据湖分层设计.md) 分层设计）；"准确 Accurate"靠质量校验（[Ch 17](./17-Landing到Raw到Redshift开发实战.md) 质量框架）。这些不是事后对照，是我设计每一层时就按 ALCOA+ 反向推导——"这个设计满足哪几条 ALCOA？"。第四年 GxP 审计，审计员看到这张"原则→实现"映射表，说"这是我见过最系统的 ALCOA 落地"——每条都能追溯到具体的工程机制，不是空喊"我们重视数据完整性"。**合规不是 PPT 里的口号，是嵌在每一层架构里的工程机制**（M10 合规从第一天嵌入）。
...
PIPL 这五条要求里，"最小必要"和"目的限制"对架构影响最深——不是"加个脱敏就行"，而是要求平台对"每个字段为什么入仓、被谁消费"有明确的声明。我在第一年做 PIPL 合规自查，发现平台里有几个字段"不知道为什么入仓了"——它们是最初批量摄取时"先都收进来再说"的产物，没有明确的消费目的。PIPL 要求"目的限制"，这些"无目的字段"就是合规风险。我花了两个月做了"字段级用途标签"治理——每个字段标注"采集目的/消费场景/脱敏策略/保留期限"，无标签字段限期下线。这个治理后来成了 [Ch 40 语义平面](./40-语义平面-三层治理与Git-YAML.md) 字段级元数据管理的雏形。**PIPL 倒逼了字段级元数据治理**——合规驱动架构演进的典型案例（M10）。
...
- 配置驱动的字段级脱敏：规则在配置中声明，SQL ELT 写入 `enriched_*` 时由 UDF 执行，不可绕过；aes_encrypt 的密钥来自 KMS 信封加密
- KMS 信封加密：CMK 永不出 KMS，Data Key 仅内存用完即焚，密钥管理与脱敏使用职责分离
- DSR 数据主体权利响应：基于血缘反追 → 分层删除（Redshift 软删 → Raw 处理 → Landing 保留标记）→ 审计报告
