...
Part V 讲完迁移与平台演进，平台本身已经成型。但故事没有结束——数据在仓库里"躺着"不会自动产生价值，它要"流向"业务系统、被业务方用起来，才有价值。Part VI 就是讲这些"把平台能力外延到业务侧"的衍生系统。

这一篇是 Part VI 的总领，先用一页讲清楚三个问题：什么是衍生系统、为什么需要、与主平台什么关系。

## 35.1 什么是衍生业务系统

衍生业务系统（Derived Business System）是**建在 CDP 数据平台之上、面向特定业务场景的消费侧系统**。它不是平台的一部分，而是平台的"用户"——从平台取数、加工成业务需要的形态、交付给业务方或回写业务系统。
...
!!! tip "引申"
    衍生系统的本质是"激活"——把平台里"就绪但静止"的数据，激活成业务可直接消费的形态。这就是行业里说的"反向 ETL（Reverse ETL）"或"数据激活（Data Activation）"。没有激活层的数据平台，就像建了水库但没修水管——水再多也流不到田里。Part VI 的两个系统，就是两条不同形态的"水管"。
...
两个系统的共性是：**都建在 CDP 平台之上，复用平台的治理能力（RLS/CLS/脱敏/审计），但各自解决一个平台不直接覆盖的业务交付问题**。阅读时可以关注它们的共同模式——"从平台取数 → 按场景加工 → 安全交付"，以及各自的特殊工程难点（低代码 + 云混合、跨云同步；API 安全与多租户隔离）。
...
!!! tip "引申"
    Data Mesh 是组织架构层面的主张（"把数据所有权下放到域"），适合超大规模企业（数千表+多独立业务线）。本书的 Aurora 规模（20000+ 表但业务域相对集中）用"中心化平台 + 衍生系统"更务实——平台统一治理，衍生系统灵活交付。如果未来 Aurora 业务域高度独立且各自有数据团队，再考虑向 Mesh 演进。这是"规模决定架构"的又一例证。
...
!!! warning "Trade-off"
    判断错误有代价——把本该建衍生系统的需求塞进平台，会让平台臃肿、职责模糊；把本该扩展平台的需求做成衍生系统，会导致治理碎片化、数据口径不一致。原则是：**平台管"通用数据能力"，衍生系统管"特定交付形态"**。拿不准时，先问"这个需求其他业务域会不会用？"——会则平台，不会则衍生。
