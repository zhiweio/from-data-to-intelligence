...
这就是 [Ch 51](./51-价值度量与案例复盘.md) 数据可观测性里提到的"数据新鲜度/质量"维度。行数对账（[Ch 17](./17-Landing到Raw到Enriched开发实战.md)）的告警不能只看"过没过"，还得看"行数是不是异常低"。平台的做法：每个实体的行数和 7 天滚动基线比，偏差超过 50% 就告警。
