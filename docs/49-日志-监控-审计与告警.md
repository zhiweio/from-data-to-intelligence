# Ch 49 日志、监控、审计与告警
!!! info "面包屑"
    [本书主页](./index.md) › [Part VIII 治理与复盘](./48-安全-合规与治理.md) › Ch 49
...
!!! tip "引申：双联章说明"
    本章与 [Ch 50 排障与可观测性实战](./50-排障与可观测性实战.md) 合构"监控排障双联章"——本章讲"怎么看见"（监控/告警/审计），Ch 50 讲"怎么排查"（决策树/walkthrough/预防）。建议两章连读。
...
```
<p class="caption" markdown="span">**图 49-1** CloudWatch/CloudTrail 与审计日志持久化</p>
...
| **Dashboards** | 可视化面板 |
<p class="caption" markdown="span">**表 49-1** CloudWatch</p>


### CloudWatch 仪表板布局设计

数据平台的仪表板不能照搬应用监控——应用关注"请求延迟/错误率"，数据平台关注"数据是否正确/及时/完整"。平台仪表板分四个分区，每区聚焦一类健康度：
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 49-2** CloudWatch 仪表板布局设计</p>
...
<p class="caption" markdown="span">**表 49-2** CloudWatch 仪表板布局设计</p>


### 结构化日志 schema 设计

数据平台的日志必须结构化——非结构化文本日志在排障时几乎无法检索。平台所有组件（Glue/Lambda/Step Functions）统一用 structlog 输出 JSON 日志，字段约定如下：
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `batch_id` | string | 批次标识，贯穿 Landing→Raw→Enriched→Redshift 全链路 |
...
<p class="caption" markdown="span">**表 49-3** 核心意图：统一字段让 CloudWatch Logs Insights 可跨组件检索</p>


排障时用 CloudWatch Logs Insights 按字段检索——比如查"ma 域最近失败的批次"：`filter domain="ma" and status="failed" | sort @timestamp desc`。没有结构化字段，这种查询 impossible。
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 49-3** CloudTrail</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 49-4** 告警体系</p>
...
<p class="caption" markdown="span">**表 49-4** 告警体系</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 49-5** 运营 Runbook</p>
...
| **验证方法** | 如何确认已恢复 |
| **事后复盘** | 记录原因+改进措施 |
<p class="caption" markdown="span">**表 49-5** 运营 Runbook</p>
...
告警疲劳是运维的隐形杀手——当每天收到 200 条告警时，真正重要的 P0 会被淹没。平台用三招治理告警疲劳：
...
<p class="caption" markdown="span">**表 49-6** 告警疲劳治理</p>
...
数据平台有一类应用平台不会遇到的可观测场景——**"ETL 成功但数据为空"**。任务返回 success，但落地行数是 0（源系统当天没数据 / 过滤条件错 / 上游断流）。传统监控只看"任务是否成功"，不会告警这种"成功但异常"：
...
<p class="caption" markdown="span">**表 49-7** 数据平台特有的可观测场景</p>


这是 [Ch 51](./51-价值度量与案例复盘.md) 数据可观测性提到的"数据新鲜度/质量"维度——行数对账（[Ch 17](./17-Landing到Raw到Enriched开发实战.md)）的告警不能只看"对账通过"，还要看"行数是否异常低"。平台的做法是：每个实体的行数与 7 天滚动基线对比，偏差超 50% 即告警。
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 49-6** 引申：数据平台可观测性 vs 应用可观测性</p>
...
<p class="caption" markdown="span">**表 49-8** 引申：数据平台可观测性 vs 应用可观测性</p>
...
!!! quote "下一章"
    [Ch 50 排障与可观测性实战](./50-排障与可观测性实战.md) —— 监控体系讲完了，接下来看实战中怎么排障。
