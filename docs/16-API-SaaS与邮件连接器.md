...
!!! tip "引申"
    认证策略的可配置化是"策略模式"的典型应用。不要为每种认证写一个客户端——而是定义统一的认证接口，不同策略实现不同接口，运行时按配置选择。这让新增一种认证方式只需加一个策略实现。
...
双向监控的价值是**端到端可观测**——无论从 CDP 侧还是 Salesforce 侧，都能看到任务的完整状态。这在跨系统协作时至关重要——"数据导出到 Salesforce 了没？"不需要登两个系统查。
