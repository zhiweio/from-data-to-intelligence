...
> **理想化设计声明**
> 本书是笔者基于自身经验构造的"理想化企业数据平台案例"。其中必要不合理之处为笔者设想的理想化设计：部分基于真实案例进行设想，部分缺乏真实验证。读者可以参考并落地验证。
...
> 这是六个边界中**最危险**的——一旦 COPY 失败，目标表为空，业务直接中断。当时的约束是快速实现加上 Redshift 事务支持有限。如果重来，Staging 表加 RENAME 切换是必须的，虽然复杂度高了，但原子性保障不能妥协。
