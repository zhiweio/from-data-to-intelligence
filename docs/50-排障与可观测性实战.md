...
理论讲再多，不如一个真实案例。这是一个典型的数据平台排障演练——某天早上一条 P1 告警："ma 域 fact_prescription 行数较基线下降 100%"。下面是完整的时间线和排查过程。
...
**第一步：确认告警**（08:00）。SNS 告警 `fact_prescription 行数较 7 天基线下降 100%`。先确认这不是误报——查 CloudWatch Logs Insights：
...
**第二步：发现矛盾**（08:10）。任务 status=success，但 row_count=0——这是 [Ch 49](./49-日志-监控-审计与告警.md) 提到的"ETL 成功但数据为空"场景。任务没失败，是"跑了但没数据"。

**第三步：查源系统**（08:15）。连源库确认 `SELECT COUNT(*) FROM prescription WHERE updated_at > '2026-06-17'`——返回 12450 行，源数据正常。问题出在 ETL 侧。
...
水位被错误地写成了未来时间（6-19，而今天是 6-18）——昨天一次手动重跑时，运维误传了参数，把水位覆盖为"明天"。于是 `WHERE updated_at > '2026-06-19'` 返回 0 行。

**第五步：修复**（08:30）。回退水位到正确值（`2026-06-17`），手动触发重跑。迟到数据回溯窗口（[Ch 14](./14-数据库与JDBC连接器.md)）确保不漏数据。

**第六步：复盘**（08:50）。行数对账通过，事件关闭。改进措施：水位更新加"不允许写未来时间"的校验——这是 [Ch 52](./52-架构师的复盘-取舍遗憾与主流对比.md) "工程诚实"的具体体现：把排障经验固化为防护栏。
...
排障不只是"功能故障"——"慢了"同样是故障。这是一个性能退化案例：用户反馈 Agentic BI 查询 P95 延迟从 3 秒涨到 30 秒，但查询结果正确。排查路径完全不同于功能故障：
...
## 50.6 预防性排障：在事故前发现问题
最好的排障是不出事。预防性排障通过"主动健康检查 + 容量预警 + 漂移检测"，在事故发生前就发现隐患：
