# Ch 50 安全、合规与治理
!!! info "面包屑"
    [本书主页](./index.md) › [Part VIII 治理与复盘](./49-评估-可观测与持续演进.md) › Ch 50
...
## 50.1 IAM 最小权限与 KMS 加密
安全合规是数据平台的免疫系统。它不是独立的一章功能，而是贯穿全书的设计线——从 [Ch 8](./08-数据仓库设计-Redshift.md) 的 RLS/CLS，到 [Ch 18](./18-数据脱敏与隐私治理.md) 的三层纵深防御，到 [Ch 29](./29-OIDC与凭证治理.md) 的 OIDC 无密钥，再到 [Ch 44](./44-五层SQL护栏与执行安全.md) 的 AI 护栏，每一层都扣在这条线上。这一章做的，是把前面散在各处的安全设计收拢成一个体系化的视图——前面拆开讲是为了跟着进度走，这里合起来是为了看清楚全局。
...
```
<p class="caption" markdown="span">**图 50-1** IAM 治理体系</p>
...
<p class="caption" markdown="span">**表 50-1** IAM 治理体系</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-2** KMS 加密</p>
...
<p class="caption" markdown="span">**表 50-2** 数据分类框架：分级保护的基础</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-3** 数据分类框架：分级保护的基础</p>

数据分类不是做完就扔的活。新字段上线必须标注分类——CI 里会校验，没标分类的字段直接阻断发布。分类变更要走 PR 审查。分类标签作为元数据存在 Glue Data Catalog 和语义资产（[Ch 40](./40-语义平面-三层治理与Git-YAML.md)）里，RLS/CLS/脱敏策略都根据分类标签自动生效。

但声明式分类盖不住一类数据：S3 里的自由文本。邮件附件、患者反馈、客服记录不进 Glue Catalog，CI 校验不到。我们在 cn-north-1 上了 AWS Macie——原生 NER + PII 发现，定期扫桶，把患者姓名、身份证号、电话这类实体捞出来，再对齐到 §50.2 的四级分类。有一次患者反馈附件里冒出未声明的 PII，Macie 直接告警。人工分类总会漏，Macie 负责兜底。
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-4** GxP ALCOA+ 原则（回顾 Ch 18）</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-5** 中国数据驻留</p>
...
<p class="caption" markdown="span">**表 50-3** 中国数据驻留</p>

### DSL 与 CSL 的工程映射

很多人一提合规就只盯 PIPL。中国这边完整一点看，是 CSL（网络安全法）、DSL（数据安全法）、PIPL，再加上出境管理办法。前三部是法律，出境管理办法是配套行政规定。PIPL 管个人信息，DSL 管数据安全，CSL 管网络安全——交叉很多。医药平台三头都沾：敏感个人信息、可能被认定为重要数据的业务数据，以及关键信息基础设施运营的嫌疑。落到工程上我是这么拆的：

- 《数据安全法》：重要数据要目录管理，出境要评估。临床试验、患者队列这类，平台得配合登记。我们的做法是在 §50.2 四级分类上给可能落入"重要数据"的 dataset 单独打标，并在网络出口盯跨境流量。
- 《网络安全法》：技术措施防攻击、防泄露；潜在关基运营者要等保 2.0 三级及以上。工程上就是网络隔离、入侵检测、日志至少留 6 个月、应急预案——跟 §50.6 的响应 runbook 对接，CloudTrail/审计日志 6 个月是硬下限。
- 出境管理办法：全球 PV 同步、全球 BI 合并这类回传，要么安全评估要么标准合同。默认全驻留 cn-north-1，没走过出境评估的 dataset 不开跨境通道。

我在企业征信项目里就吃过"只盯 PIPL"的亏。当时合规精力全砸在个人信息上，DSL 出台后才发现企业征信数据也可能算重要数据，没做目录登记，被监管约谈。到 Aurora 我把"三法一架"当成每年自查清单，哪部法改了就重新过一遍平台（M10 合规嵌入）。

---

## 50.4 Redshift RLS/CLS 策略在数据治理中的深度应用
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-6** Redshift RLS/CLS 策略在数据治理中的深度应用</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-7** RLS 策略绑定与租户隔离</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-8** CLS 列级权限与敏感字段控制</p>
...
| 敏感 | 合规角色 | 患者姓名、电话 |
| 极敏感 | 管理员（脱敏后） | 患者身份证号 |
<p class="caption" markdown="span">**表 50-4** CLS 列级权限与敏感字段控制</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-9** RLS/CLS 与审计日志联动</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-10** 治理最佳实践与平台护栏</p>
...
<p class="caption" markdown="span">**表 50-5** 治理最佳实践与平台护栏</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-11** 安全事件响应：从告警到复盘的全流程</p>
...
<p class="caption" markdown="span">**表 50-6** 示意：安全事件 runbook 模板</p>
...
<p class="caption" markdown="span">**表 50-7** 灾难恢复与业务连续性</p>
...
linkStyle default stroke:#697077,stroke-width:2px
```
<p class="caption" markdown="span">**图 50-12** 灾难恢复与业务连续性</p>
...
<p class="caption" markdown="span">**表 50-8** 灾难恢复与业务连续性</p>


!!! warning "Trade-off"
    跨区域复制（CRR）会把存储成本翻倍，China 的 DR 区域选择也有限（只有 cn-north-1 / cn-northwest-1）。我们只对"极敏感 + 不可重建"的数据做 CRR（Landing 原始层、审计日志）。仓内 `enriched_*` / Curated 能从 Raw 重跑 SQL ELT 恢复，就不做 CRR。不是所有数据都值得跨区域复制，先把不可重建的认出来。
...
- "三法一架"落到工程：CSL（等保 2.0 三级、日志留 6 个月）+ DSL（重要数据目录、出境评估）+ PIPL + 出境管理办法。别只盯 PIPL
...
!!! quote "下一章"
    [Ch 51 日志、监控、审计与告警](./51-日志-监控-审计与告警.md) —— 安全治理讲完了，接下来看日常运维的监控审计体系。
