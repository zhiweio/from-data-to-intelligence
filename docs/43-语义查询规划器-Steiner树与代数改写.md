...
flowchart LR
 ...
 S4@{ icon: "codicon:graph", form: "rounded", label: 语义中间层<br/>Steiner 树规划+代数改写<br/>the-ttd 方案, pos: "b", h: 36 }
 end
...
!!! tip "引申"
    Steiner 树是图论中的经典 NP-hard 问题——给定图中一组"终端节点"，找到连接它们的最小代价子树。KMB（Kou-Markowsky-Berman）算法是常用的近似算法。在 NL2SQL 中，"终端节点"是问题涉及的表，"Steiner 树"是最优 join 路径。把图论算法引入 NL2SQL 是 the-ttd 的关键创新——它让 join 路径选择从"LLM 猜"变为"算法算"。
...
"把 join 路径选择从 LLM 猜变为算法算"是全书最重要的技术声明，必须有数据支撑。以下是 the-ttd 引入 Steiner 树规划器前后的端到端准确率对比（数据按比例缩放自真实评测，规模量级保留）：
...
!!! tip "引申"
    幻觉分类学的价值在于"对症下药"——幻觉列靠检索纠错（成功率最高，90%+），幻觉值靠术语绑定，幻觉 join 靠规划器重算，幻觉聚合靠规则约束。把幻觉从"LLM 的随机错误"变为"可分类、可针对性修复的工程问题"，是 the-ttd 自愈成功率能到 ~70% 的关键（详见 [Ch 47](./47-评估-可观测与持续演进.md) 基准评测）。
...
!!! tip "引申"
    这三大陷阱是 NL2SQL 在生产中失败的根因。大多数 NL2SQL demo 在"简单单表查询"上表现很好，但遇到"多表 join + 聚合"就翻车——不是因为 LLM 不够聪明，而是因为 join 语义的复杂性超出了"纯文本生成"的能力范围。the-ttd 通过"规划器先算+护栏后验"把这些问题从"LLM 的责任"变为"工程系统的责任"。
...
- 三大经典陷阱：鸿沟陷阱（重复计数）/ 扇出陷阱（行数膨胀）/ 幻觉列（不存在的列名）——the-ttd 用规划器+护栏分别应对
