...
这个问题在 Agentic BI 建设初期让我吃了大亏。最初版本的 NL2SQL 流程很简单：RAG 检索表结构 → LLM 生成 SQL → 执行。在简单查询上（"上个月总处方量"），它工作得很好。但当用户问复杂问题（"华东区 CardioBrand-A 上月处方量按医院等级分布"）时，LLM 经常生成错误的 join——要么 join 了多余的表导致扇出，要么 join 顺序错误导致鸿沟陷阱（重复计数），要么漏 join 了必要的维度表。

问题的根源是：**join 路径选择不是"文本生成"问题，而是"图论"问题。** 数仓里的表间关系构成一个图，用户问题涉及的表是图中的节点，找到连接这些表的最优路径是一个 Steiner 树问题。让 LLM "猜" join 路径，不如用算法"算"出来。

这个认知让我决定引入语义查询规划器——在 LLM 生成 SQL 之前，先用图论算法算出最优 join 子图，再让 LLM 在这个"已经规划好的路径"上生成 SQL。这把 NL2SQL 的可靠性提升了一个数量级。

!!! tip "引申：为什么 join 是 NL2SQL 的最大难点？"
    在自然语言理解中，"join"对应的是"关系推理"——用户说"华东区 CardioBrand-A 的处方量"，AI 需要推理出"处方量在事实表里、区域在维度表里、药品也在维度表里、三者通过外键关联"。这对人来说很直觉，但对 LLM 来说极难——因为它需要同时理解表结构、外键关系和业务语义。而且 join 路径可能不止一条（直接 join 或通过中间表 join），选错路径会导致结果错误或性能极差。这就是为什么我们用图论算法（而非 LLM 推理）来解决 join 路径选择。
...
!!! tip "引申"
    Steiner 树是图论中的经典 NP-hard 问题——给定图中一组"终端节点"，找到连接它们的最小代价子树。KMB（Kou-Markowsky-Berman）算法是常用的近似算法。在 NL2SQL 中，"终端节点"是问题涉及的表，"Steiner 树"是最优 join 路径。把图论算法引入 NL2SQL 是 NewtonData 的关键创新——它让 join 路径选择从"LLM 猜"变为"算法算"。
...
引入 Steiner 树后，复杂查询准确率提升 12 个百分点——这 12 个百分点正是"鸿沟陷阱"和"错误 join 条件"两类失败被消除的结果。对含桥接表的查询，提升更显著（24%），因为 LLM 几乎无法猜对"需要经过哪张中间表才能连接两张表"。
...
!!! tip "引申"
    KMB 的实测表现远好于理论上界，原因是真实数仓的表关系图是"稀疏图"——表间 join 关系有限，不像理论最坏情况那样稠密。这意味着 KMB 在实践中几乎等价于最优解，近似误差可忽略。这也是为什么我们选择 KMB 而非更复杂精确算法——性价比最高。
...
!!! tip "引申：基石回扣——Steiner 树的图 = Kimball 星型模型"
    Steiner 树操作的"表关系图"本质上就是 [Ch 8](./08-数据仓库设计-Redshift.md) 建立的 Kimball 星型模型——事实表（fact）为中心，维度表（dim）环绕，通过外键关联。Steiner 树的终端节点（terminal nodes）就是查询涉及的事实表和维度表，边就是 join 路径，边权就是 join 代价。

    四种 cost 信号都有基石来源：FK 基数来自 [Ch 20](./20-元数据管理与数据血缘.md) 的 Glue Catalog 元数据；手动标注来自 [Ch 40](./40-语义平面-三层治理与Git-YAML.md) 的 Join 语义资产（`cost: 1.0`）；查询历史来自审计日志（[Ch 11](./11-配置与状态管理.md) batch_id 可追溯）。Steiner 树不是凭空建图——它消费的是 CDP 平台前三年的元数据积累。**AI 规划器站在数据平台的肩膀上**。
...
!!! warning "Trade-off"
    代数改写的价值是"保证 SQL 正确性和性能"，但代价是规划器复杂度大幅增加。对于简单查询（单表+简单聚合），代数改写是过度工程；但对于复杂分析查询（多表 join+聚合），代数改写是避免"鸿沟陷阱"的必需品。规划器通过 Router 判断查询复杂度，只对复杂查询启用改写。
...
!!! tip "引申"
    幻觉分类学的价值在于"对症下药"——幻觉列靠检索纠错（成功率最高，90%+），幻觉值靠术语绑定，幻觉 join 靠规划器重算，幻觉聚合靠规则约束。把幻觉从"LLM 的随机错误"变为"可分类、可针对性修复的工程问题"，是 NewtonData 自愈成功率能到 ~70% 的关键（详见 [Ch 47](./47-评估-可观测与持续演进.md) 基准评测）。
...
!!! tip "引申"
    这三大陷阱是 NL2SQL 在生产中失败的根因。大多数 NL2SQL demo 在"简单单表查询"上表现很好，但遇到"多表 join + 聚合"就翻车——不是因为 LLM 不够聪明，而是因为 join 语义的复杂性超出了"纯文本生成"的能力范围。NewtonData 通过"规划器先算+护栏后验"把这些问题从"LLM 的责任"变为"工程系统的责任"。
